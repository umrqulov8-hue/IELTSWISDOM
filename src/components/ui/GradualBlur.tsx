...
const mergeConfigs = (...configs: any[]): Config => configs.reduce((acc, c) => ({ ...acc, ...c }), {});
...
const useResponsiveDimension = (responsive: boolean, config: Config, key: string) => {
  ...
  const blurDivs = useMemo(() => {
    ...
    for (let i = 1; i <= config.divCount; i++) {
        ...
        if (config.exponential) {
          blurValue = ((Math.pow(2, progress * 4) * 0.0625) as any) * currentStrength;
        } else {
