...
import { User, Camera, Languages, Type, Check, X, LogOut, Crown, Zap, ChevronRight, Palette } from "lucide-react";
...
export function UserSettingsPanel({ isOpen, onClose }: UserSettingsPanelProps) {
    ...
    const [themeVal, setThemeVal] = useState<"light" | "dark" | "dim">("light");
    const [cPrev, setCPrev] = useState<"1" | "2" | "3">("1");

    useEffect(() => {
        const t = document.documentElement.getAttribute('data-theme') || "light";
        setThemeVal(t as "light" | "dark" | "dim");
        setCPrev(t === "dark" ? "2" : t === "dim" ? "3" : "1");
    }, []);

    const handleThemeChange = (newTheme: "light" | "dark" | "dim") => {
        const currentOpt = themeVal === "light" ? "1" : themeVal === "dark" ? "2" : "3";
        setCPrev(currentOpt);
        setThemeVal(newTheme);
        document.documentElement.setAttribute('data-theme', newTheme);
        if (newTheme === 'dark' || newTheme === 'dim') document.documentElement.setAttribute('data-dark-mode', 'true');
        else document.documentElement.setAttribute('data-dark-mode', 'false');
    };
        ...
        <AnimatePresence>
            {isOpen && (
                <motion.div
                    ...
                    animate="visible"
                    exit="exit"
                    className="absolute left-[70px] top-[70px] w-[300px] bg-white/40 backdrop-blur-[40px] border border-white/60 z-[120] overflow-hidden flex flex-col ring-1 ring-white/50 rounded-3xl"
                >
                    ...
                    <div className="p-2 flex flex-col gap-1">
                        ...
                        <motion.div variants={{ hidden: { opacity: 0, y: 15, scale: 0.95 }, visible: { opacity: 1, y: 0, scale: 1, transition: { duration: 0.4, ease: [0.34, 1.56, 0.64, 1] } } }} className="p-3 bg-white/50 backdrop-blur-xl rounded-2xl shadow-sm border border-white/60">
                            ...
                            <fieldset className="switcher" data-previous={cPrev} style={{ width: '100%', maxWidth: '100%' }}>
                                <legend className="switcher__legend">Choose theme</legend>

                                <label className="switcher__option">
                                    <input className="switcher__input" type="radio" name="theme" value="light" data-option="1" checked={themeVal === "light"} onChange={() => handleThemeChange("light")} />
                                ...
                                <label className="switcher__option">
                                    <input className="switcher__input" type="radio" name="theme" value="dark" data-option="2" checked={themeVal === "dark"} onChange={() => handleThemeChange("dark")} />
                                    <svg className="switcher__icon" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 36 36">
                                ...
                                <label className="switcher__option">
                                    <input className="switcher__input" type="radio" name="theme" value="dim" data-option="3" checked={themeVal === "dim"} onChange={() => handleThemeChange("dim")} />
                                    <svg className="switcher__icon" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 36 36">
