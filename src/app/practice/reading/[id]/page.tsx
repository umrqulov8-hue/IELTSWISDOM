                        ...
                        <div className="w-full md:w-[40%] bg-white rounded-3xl p-8 shadow-sm border border-slate-100 overflow-y-auto custom-scrollbar">
                            ...
                            <div className="space-y-8">
                                {testData.questions.map((q) => {
                                    ...
                                    // Special Handling for "Raising the Mary Rose" Diagram (Q9-13)
                                    if (testId === "fp-3" && q.id >= 9 && q.id <= 13) {
                                        if (q.id === 9) {
                                            const checkCorrect = (id: number) => {
                                                const question = testData.questions.find(x => x.id === id);
                                                if (!question) return false;
                                                const userAnswer = answers[id];
                                                if (typeof userAnswer === 'string') {
                                                    // Allow alternative answers listed in the prompt if we split by comma
                                            ...
                                            return (
                                                <div id="question-9" key="mary-rose-diagram" className="mb-12 rounded-3xl overflow-hidden shadow-xl border border-slate-200 relative bg-white pb-6">
                                                    <div className="p-6 border-b border-slate-100 bg-slate-50 relative z-10">
                                                        <h4 className="font-bold text-slate-800 text-lg">Questions 9-13</h4>
                                                        <p className="text-slate-600">Label the diagram below. Choose ONE WORD ONLY from the passage for each answer.</p>
                                                    </div>
                                                    <div className="relative w-full max-w-2xl mx-auto mt-6 z-0">
                                                        ...
                                                        <div id="question-13" className="absolute" style={{ top: '83.5%', left: '30%', width: '18.5%', height: '3.6%' }}>
                                                            <input type="text"
                                                                ...
                                                                placeholder="13..."
                                                            />
                                                            {isSubmitted && !checkCorrect(13) && <div className="absolute top-full left-0 mt-1 bg-white border border-red-200 text-red-600 text-[10px] px-2 py-0.5 rounded shadow whitespace-nowrap z-50">Answer: {String(testData.questions.find(x => x.id === 13)?.correctAnswer)}</div>}
                                                        </div>
                                                    </div>
                                                </div>
                                            );
                                        } else {
                                            return null; // Skip rendering for 10-13 as they are in the diagram
                                        }
                                    }
